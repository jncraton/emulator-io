...
// Define operations
typedef enum {Halt=0, LoadImmediate, Add, AddImmediate, And, Or, Xor, Jump, BranchOnEqual, Move, LoadDirect, StoreDirect, LoadIndirect, StoreIndirect, SleepImmediate, Invalid}  __attribute__ ((__packed__)) Operation;

// Define plain-text instruction names for use in assembler
char* instruction_names[15] = {"Halt", "LoadImmediate", "Add", "AddImmediate", "And", "Or", "Xor", "Jump", "BranchOnEqual", "Move", "LoadDirect", "StoreDirect", "LoadIndirect", "StoreIndirect", "SleepImmediate"};
...
void execute(int32_t * memory, int * registers) {
  ...
  while (inst.op != Halt) {
    ...
    } else if (inst.op == SleepImmediate) {
      struct timespec delay;
      delay.tv_nsec = (inst.immediate % 100) * 1e7;
      delay.tv_sec = (inst.immediate / 100);
...
int main() {
  ...
  printf("Marquee Program\n");
  f = fopen("marquee.asm", "r");
  memset(program, 0, 10000);
  fread(program, 1, 10000, f);
